...
- **`internal/style`**: 색을 hex 값이 아니라 역할(`Hash`, `Danger`, `Meta` 등)로 노출하며,
  Unicode/ASCII 글리프 테이블과 ANSI 인식 폭 계산 헬퍼를 함께 제공합니다. 덕분에 `git`이 이미 색을 입혀
  내려주는 diff 출력도 레이아웃을 깨뜨리지 않습니다.
